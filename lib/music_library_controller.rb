class MusicLibraryController
    ...
    def list_songs_by_artist
        ...
        if Artist.find_by_name(artist)
            Song.all.select{|song|song.artist.name == artist}.sort_by{|song|song.name}.each_with_index{|song, i| puts "#{i+1}. #{song.name} - #{song.genre.name}"}
        end
    end
    ...
    def list_songs_by_genre
        ...
        if Genre.find_by_name(genre)
            Song.all.select{|song|song.genre.name == genre}.sort_by{|song|song.name}.each_with_index{|song, i| puts "#{i+1}. #{song.artist.name} - #{song.name}"}
        end
    end     
    ...
    def play_song
        ...
        num = gets.chomp.to_i
        if num.between?(1,Song.all.size)
            song = Song.all.sort_by{|song|song.name}[num - 1]
